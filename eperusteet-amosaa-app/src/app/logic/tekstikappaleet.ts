...
namespace Tekstikappaleet {
    ...
    export const paivitaRakenne = (viite: Tekstikappaleeviite) => {
        _(viite.lapset)
            .forEach((lapsi: Tekstikappaleeviite) => {
                lapsi.$$parent = viite;
    ...
    export const poista = (viite: Tekstikappaleeviite) => {
        if (viite.$$obj.tyyppi == "tutkinnonosat" || viite.$$obj.tyyppi == "suorituspolut" || viite.$$obj.pakollinen) {
            ...
            NotifikaatioService.varoitus(
                viite.$$obj.pakollinen ? "pakollinen-ei-saa-poistaa" : viite.$$obj.tyyppi + "-ei-voi-poistaa"
            );
        } else {
            viite.$$poistettu = true;
        }
        _(viite.lapset)
            .forEach(lapsi => poista(lapsi))
            .value();
    };
    ...
    export const poistaPoistetut = (viite: Tekstikappaleeviite) => {
        ...
        _(viite.lapset)
            .forEach(viite => poistaPoistetut(viite))
            .value();
        return viite;
    };
}
