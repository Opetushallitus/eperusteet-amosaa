...
@Service
@Transactional
public class KotoLaajaAlainenOsaaminenSisaltoviiteServiceImpl implements SisaltoViiteToteutusService {
    ...
    public SisaltoTyyppi getSisaltoTyyppi() {
        return SisaltoTyyppi.KOTO_LAAJAALAINENOSAAMINEN;
    }

    @Override
    public SisaltoViite updateSisaltoViite(SisaltoViite viite, SisaltoViiteDto uusi) {
        KotoLaajaAlainenOsaaminen uusiKotoLao = mapper.map(uusi.getKotoLaajaAlainenOsaaminen(), KotoLaajaAlainenOsaaminen.class);
        viite.setKotoLaajaAlainenOsaaminen(uusiKotoLao);
        return viite;
    }
}
